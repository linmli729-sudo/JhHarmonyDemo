...
def build():
    ...
    body(doc, "本次共執行 205 條功能測試案例，統計結果如下（數字請依實際執行填入）。")
    ...
    body(doc, "〔請貼上：各模組執行結果長條圖（來源：Excel）〕", red=True)
    caption(doc, "圖4-1　各模組測試案例執行結果長條圖", above=False)
    heading(doc, "4.2 缺陷管理", 2)
    body(doc, "本次共發現 〔X〕 個有效缺陷（作業要求不少於 5–8 個），清單如下，截圖請附於各缺陷後或統一附錄。")
    ...
    make_table(doc, ["Bug ID", "標題", "嚴重度", "優先級", "狀態", "重現步驟摘要"], [
        ...
        ["BUG-004", "〔填寫〕", "〔〕", "〔〕", "〔〕", "〔填寫〕"],
        ["BUG-005", "〔填寫〕", "〔〕", "〔〕", "〔〕", "〔填寫〕"],
    ], widths=[2, 3.5, 1.6, 1.6, 1.6, 4.1], body_size=9, header_size=9)
    body(doc, "〔請貼上：缺陷類型分布餅狀圖（來源：Excel）〕", red=True)
