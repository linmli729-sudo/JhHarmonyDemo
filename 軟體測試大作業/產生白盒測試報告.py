...
def build():
    ...
    body(doc, "白盒測試（又稱結構測試）以程式的內部邏輯結構為依據設計測試用例，要求測試人員了解原始碼的控制流程。本報告針對 Automation Exercise 電子商務系統之兩個核心邏輯模組——「購物車訂單金額計算」與「商品價格冒泡排序」——繪製程式流程圖，並依三種邏輯覆蓋準則設計測試用例，量化覆蓋程度：")
    body(doc, "（1）語句覆蓋（Statement）：使程式中每一條可執行語句至少被執行一次，強度最弱。")
    body(doc, "（2）判定覆蓋（Decision/Branch）：使每個判定（分支）的取真與取假至少各執行一次，強度較語句覆蓋為強。")
    body(doc, "（3）條件覆蓋（Condition）：使每個判定中每一個原子條件的真、假取值至少各出現一次，著重複合判定中各原子條件的取值，常與判定覆蓋互補。")
    body(doc, "三者強弱關係為「語句覆蓋 ＜ 判定覆蓋」，本報告對含複合條件的判定（D3）特別進行條件覆蓋設計。")
    heading(doc, "1.2 被測核心模組", 2)
    body(doc, "模組一 calc_order_amount——依會員等級折扣、滿額立減、優惠券計算購物車結帳之訂單應付金額；模組二 bubble_sort_by_price——將商品依價格升序排序（冒泡排序，含「本趟未交換即提前結束」之最佳化），對應商品排序顯示。")
    ...
    body(doc, "本模組共有 4 個判定節點：D1（member_level == 2，是否 VIP，折扣 0.90）、D2（member_level == 1，是否一般會員，折扣 0.95）、D3（member_level≥1 且 subtotal≥1000，複合條件，C1＝會員、C2＝滿千，成立則立減 100）、D4（total < 0，金額是否為負而需歸零）。其中 D3 為複合條件判定，包含兩個原子條件 C1、C2，故另作條件覆蓋設計。")
    ...
    heading(doc, "3.3 語句、判定與邊界覆蓋測試用例", 2)
    body(doc, "本模組含 4 個判定：DB1（外層 i<n-1）、DB2（內層 j<n-1-i）、DB3（相鄰價格比較）、DB4（本趟未交換則 break）。下表用例使全部語句執行一次、各判定真假分支皆覆蓋，並涵蓋零次、一次、相等元素等邊界與迴圈情形。")
    caption(doc, "表B-4　語句／判定／邊界覆蓋測試用例")
    make_table(doc, ["用例", "輸入（名稱,價格）", "涵蓋重點", "預期輸出（價格序）"], [
        ["TC-S01", "[(A,30),(B,20),(C,10)]（完全反序）", "多次交換，DB1-DB4 真假皆覆蓋（DB4=F）", "10,20,30"],
        ["TC-S02", "[(A,10),(B,20),(C,30)]（已序）", "首趟無交換→break 提前結束（DB4=T）", "10,20,30"],
        ["TC-S03", "[(A,5)]（單一元素）", "外層迴圈零次，直接 return（DB1=F）", "5"],
        ["TC-S04", "[]（空串列，n=0）", "邊界：迴圈執行 0 次，不崩潰", "[]"],
        ["TC-S05", "[(A,10),(B,10),(C,5)]（含相等）", "DB3 取「等於」邊界（>為假，不交換）", "5,10,10（穩定）"],
    ], widths=[1.7, 5.0, 6.0, 3], body_size=9, header_size=9.5)
    body(doc, "綜合上述用例：語句 B1～B8 全部覆蓋（B6 交換由 S01、B7 break 由 S02），DB1～DB4 之真假分支皆覆蓋，語句與判定覆蓋率均達 100%。", size=11)

    # 四、覆蓋率匯總與結論
    heading(doc, "四、覆蓋率匯總與結論", 1)
    body(doc, "覆蓋率匯總：calc_order_amount（4 個判定，5 條用例 W01-W05）之語句覆蓋、判定覆蓋、條件覆蓋率均達 100%；bubble_sort_by_price（4 個判定，5 條用例 S01-S05，無複合條件）之語句覆蓋與判定覆蓋率均達 100%。")
